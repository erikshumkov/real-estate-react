...
import { sortByNewest, getHomeData, getCityData } from "../../actions/itemActions"

import FilterSection from '../layout/FilterSection'
import Spinner from '../layout/Spinner'


const MapPage = ({ location, item: { filteredData, search, data, cityData, loading }, sortByNewest, getHomeData, getCityData }) => {
  ...
  const enterLi = (id) => {
    let style = mapItemRefs[id].current._icon.firstChild.style;
    initZindex = mapItemRefs[id].current._icon.style.zIndex;
    mapItemRefs[id].current._icon.style.zIndex = 1000;
  ...
  const leaveLi = (id) => {
    let style = mapItemRefs[id].current._icon.firstChild.style;
    mapItemRefs[id].current._icon.style.zIndex = initZindex;
      ...
      <div className="MapPage">
        <div className="grid">
          <div className="items">
            ...
            {loading && data.length === 0 ? <Spinner /> : filteredData.map(home => (
              <Link
                key={home.id}
                ...
                className="home-link"
                to={`/item/${home.route[0].city}/${home.route[0].address}`}
              >
                <div className="home-item">
                  ...
                  <div className="text">
                    ...
                    <div className="text-info">
                      ...
                      <span className="info-size">Boarea: {home.sqmeter} kvm</span>
            ...
            {filteredData.length === 0 && !loading && (
              <h4>Hittade inget. Testa söka efter något annat.</h4>
            )}

          </div>
          <div className="map-container">
            {loading && data.length === 0 && cityData.length === 0 ? null : <MapRealEstate key={filteredData.id} location={location} getItemOnClick={getItemOnClick} mapItemRefs={mapItemRefs} data={filteredData} />}
          </div>
        </div>
      </div>
    </>
...
export default connect(mapStateToProps, { sortByNewest, getHomeData, getCityData })(MapPage)
